...
extern void out_check(input bit[3-1:0] CurLayer, input bit[12-1:0] IPOutAddr, input bit[32-1:0] IPOutData, inout bit[32-1:0] SigEnergy, inout bit[32-1:0] ErrEnergy, output bit ErrFlag);
...
extern pointer  wrFileDescriptor(input string fileName);
extern void write2File(input pointer fileDescriptor, input bit[12-1:0] wrAddr, input bit[16-1:0] wrData);
...
(
    ...
    input  [9-1:0]  sim_ip_buf_addr_i //data addr in ip_buf, 0~WL-1
);
    ...
    integer fd_data;
    initial begin
        fd_data = wrFileDescriptor("./fc8_sim_fp16.txt");
        if(fd_data == 0) begin
          $display("fc8_sim_fp16 opened wrong\n");
          $finish;
        end
    end
...
//    end
    always @(sim_ip_buf_addr_i or block_pos) begin
        file_pos = block_pos + {23'd0, sim_ip_buf_addr_i} + 32'd7359456;
    end
    ...
    always @(negedge sim_rstn_i or posedge sim_clk_i) begin
        if(~sim_rstn_i) begin
            ...
            if(sim_ip_data_in_valid_i && (sim_cur_layer_index_i==3'd0)) begin
                in_flag = data_in_check(sim_ip_data_in_i, ip_data_pos, pixel_pos_reg_sim, channel_pos_reg_sim);
            end
            ...
            */
            ...
            if(sim_ip_out_valid_i && (sim_cur_layer_index_i==3'd4)) begin
                write2File(fd_data, sim_ip_addr_out_i, sim_ip_data_out_i);
            end
