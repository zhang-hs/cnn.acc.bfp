// ---------------------------------------------------
// File       : rd.param.mem.ker.bias.tb.v
//
// Description: read param data from ddr, write to mem_bias
//              and mem_ker module, read only once
//
// Version    : 1.0
// ---------------------------------------------------
...
`define usingDirectC
`ifdef usingDirectC
  extern void printBias(input bit[16384-1:0] biasData, input bit[5:0] numOfBiasBurst);
  extern void printWeight(input bit[32*3*3*32-1:0] weightData);
`endif
module top;
   //***************************************************************************
  ...
  initial begin
    fd_data = $fopen("../../../data/param.mem.ker.bias.tb.bin","rb");
    char_count = 0;
    data_count = 0;
  ...
  localparam WR_END_ADDR = 30'h138; // (bias_num + 32*9*2)*float_num_bit/ddr_data_width - ddr_burst_size
  ...
  always@(posedge ddr_clk) begin
    if(sync_rst) begin
      ...
      if(_wr_addr[11:0] == WR_END_ADDR) begin
        tb_load_done <= 1'b1;
  ...
  wire        rd_param_valid;
  wire        rd_param_full;
  wire        rd_param_bias_valid;
  wire        rd_param_bias_valid_last;
  wire        rd_param_last;
  wire [511:0]rd_param_data;
  wire [16384-1:0]      mem_bias_o;
  wire [32*3*3*32-1:0]  mem_ker_o;
  ...
  localparam BIAS_DATA_BURST_NUM = 6'd4;

  rd_ddr_param rd_param(
      .clk(ddr_clk),
      .rst_n(sys_rst_n),
      .ddr_rdy(ddr_rdy),
      .ddr_rd_data_valid(ddr_rd_data_valid),
      .ddr_addr(rd_ddr_addr), // output
      .ddr_cmd(rd_ddr_cmd), // output
      .ddr_en(rd_ddr_en), // output
      .ddr_rd_data(ddr_rd_data),
      .rd_param(tb_load_done),
      .rd_param_ker_only(1'b0),
      .rd_param_bias_burst_num(BIAS_DATA_BURST_NUM), // number of conv layer bias data,
      .rd_param_addr(30'h00),
      .rd_param_valid(rd_param_valid), // output
      .rd_param_bias_valid(rd_param_bias_valid),
      .rd_param_last(rd_param_last),
      .rd_param_bias_valid_last(rd_param_bias_valid_last),
      .rd_param_data(rd_param_data),
      .rd_param_full(rd_param_full) // output
    );

  mem_bias bias_data(
      .clk(ddr_clk),
      .rst_n(sys_rst_n),
      .mem_bias_valid(rd_param_valid && rd_param_bias_valid),
      .mem_bias_last(rd_param_bias_valid_last),
      .mem_bias_i(rd_param_data),
      .mem_bias_o(mem_bias_o)
    );
  mem_ker weight_data(
      .clk(ddr_clk),
      .rst_n(sys_rst_n),
      .mem_ker_valid(rd_param_valid && (!rd_param_bias_valid)),
      .mem_ker_last(rd_param_last),
      .mem_ker_i(rd_param_data),
      .mem_ker_o(mem_ker_o)
    );
  ...
  reg printDone;
  always@(posedge ddr_clk) begin
    if(rd_param_bias_valid_last) begin
      cmp_bias <= 1'b1;
    end else begin
      cmp_bias <= 1'b0;
    end
    if(rd_param_last) begin
      cmp_ker <= 1'b1;
    end else begin
      cmp_ker <= 1'b0;
    end
    if(cmp_bias) begin
    // compare bias data from mem_bias module, by using directC
      printBias(mem_bias_o, BIAS_DATA_BURST_NUM);
    end
    if(cmp_ker) begin
    // compare ker data from mem_ker module, by using directC
      printWeight(mem_ker_o);
      printDone <= 1'b1;
    end else begin
      printDone <= 1'b0;
    end
