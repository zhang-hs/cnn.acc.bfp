...
module top;
  ...
  initial begin
    fd_data = getFileDescriptor("../../data/bottom.conv1_2.txt");
    fd_param= getFileDescriptor("../../data/param.conv1_2.txt");
    char_count = 0;
    data_count = 0;
  ...
  localparam WR_DATA_END_ADDR   = {DDR_BANK_PATCH, DDR_ROW, DDR_COL} + 30'h1ffff8; // 30'h17ff8; // 30'h61f8; // channels*(fm_width*fm_height)*float_num_bit/ddr_data_width - ddr_burst_size
  localparam WR_PARAM_START_ADDR= {DDR_BANK_PARAM, DDR_ROW, DDR_COL};
  localparam WR_KER_START_ADDR  = WR_PARAM_START_ADDR + 30'd32;
  localparam WR_PARAM_END_ADDR  = {DDR_BANK_PARAM, DDR_ROW, DDR_COL} + 30'h4818; // 30'h378; // (bias_num + 64*3*9)*float_num_bit/ddr_data_width - ddr_burst_size
...
//wire conv_ddr_rd_data_valid; -> ddr_rd_data_valid;
  ...
  // -------------------- simulation --------------------}
      ...
      // layer configuration
      .fm_relu_en(1'b1), // <-x enable relu, should change the output data for top data checking
      .fm_pooling_en(1'b1),
      // bottom
      ...
      .fm_bottom_channels(10'd64), // num of bottom data channels
