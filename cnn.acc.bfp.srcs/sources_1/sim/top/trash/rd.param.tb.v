...
// File       : rd.param.tb.v
//
// Description: read param data from ddr, write to mem_bias and mem_ker module
//
// Version    : 1.0
// ---------------------------------------------------
...
module top;
  ...
  initial begin
    fd_data = $fopen("../../data/param.tb.bin","rb");
    char_count = 0;
    data_count = 0;
  ...
  rd_ddr_param rd_param(
      ...
      .rd_ker_only(1'b0),
      .rd_bias_burst_num(6'd4), // number of conv layer bias data,
      .rd_param_addr(30'hb0),
      .rd_param_valid(rd_param_valid), // output
      .rd_param_full(rd_param_full) // output
    );

  always@(posedge ddr_clk) begin
    if(sys_rst_n && rd_param_full)
      $finish;
  end
