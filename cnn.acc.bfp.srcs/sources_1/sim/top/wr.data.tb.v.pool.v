...
module top;
    ...
    begin
      ...
      for(idx_r=0; idx_r<ATOMIC_H/2; idx_r=idx_r+1) begin
        for(idx_c=ATOMIC_W/2; idx_c<ATOMIC_W; idx_c=idx_c+1) begin
          if(pool_en) begin
            _wr_data[idx_r*ATOMIC_W + idx_c] = {32{1'b1}};
          end else begin
            _wr_data[idx_r*ATOMIC_W + idx_c] = {wr_channel_index[7:0], wr_y[7:0], wr_x[7:0], idx[7:0]}; // {wr_channel_index[7:0], wr_y[7:0], wr_x[7:0], idx[7:0]};
            idx = idx+1;
          end
        end
      end
  ...
  localparam  WR_END_X       = 0;
  localparam  WR_END_Y       = 0;
  localparam  WR_HALF_BAR_SZ = 30'h40; // half bar size, 64*2*num_of_atom_in_bar*data_width/ddr_data_width
//localparam  WR_FM_SZ       = 30'h800; // feature map size, 64*4*num_of_atom_in_bar*num_of_bars*data_width/ddr_data_width
  // pooling
  localparam  WR_FM_SZ       = 30'h20; // feature map size, 64*4*num_of_atom_in_bar*num_of_bars*data_width/ddr_data_width
  ...
  initial begin
    _wr_pooling_en = 1'b1;
    TASK_GEN_DATA(_wr_pooling_en, 9'h0, 9'h0, 10'h0);
  end
  ...
  always@(posedge ddr_clk or negedge sys_rst_n) begin
    if(!sys_rst_n) begin
      ...
      if(_wr_data_next_channel) begin
        ...
        TASK_GEN_DATA(_wr_pooling_en, _gen_data_x, _gen_data_y, _gen_channel_idx);
      end
    end
  end
  // channel data
...
//genvar i;
  ...
  wr_ddr_data wr_data_instance(
    ...
    .wr_data_pooling_en(_wr_pooling_en), // is pooling layer output
