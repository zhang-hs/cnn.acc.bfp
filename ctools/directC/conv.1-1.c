...
#define fmHeight        224 //bottom fm height, the top one is half only pooling is performed
#define fmWidth         224
#define fmRow           16
#define fmCol           16
#define kerChannels     64
#define kerWidth        3
#define kerHeight       3
#define channels   3  // number of bottom channels // <-x
#define biasNUM    64 // number of top channels
#define biasMax    512
#define epsilonError  4.0 // 5_3 -> 8.0, 4-3 -> 14.0, 4_1 -> 32.0, 3_3 -> 32.0, 3_2 -> 16.0, 3_1 -> 16.0, 2_2 -> 16.0, 2_1 -> 12.0, 1_2 -> 4.0 , 1_1 -> 4.0
#define relativeError 0.1 // 5_3 -> 0.2, 4-3 -> 0.15, 4_1 -> 0.25, 3_3 -> 0.25, 3_2 -> 0.25, 3_1 -> 0.25, 2_2 -> 0.25, 2_1 -> 0.15, 1_2 -> 0.1 , 1_1 -> 0.1
#define absError      0.4 // 5_3 -> 2.0, 4-3 -> 3.6 , 4_1 -> 6.8 , 3_3 -> 6.8 , 3_2 -> 4.7 , 3_1 -> 4.7 , 2_2 -> 4.7 , 2_1 -> 2.3 , 1_2 -> 0.45, 1_1 -> 0.4
...
void write2File(void* fileDescriptor, U wrEnable, U poolEn, U posX, U posY, U quarterIdx, U channelIdx, U* wrData)
{
  unsigned int offset;
  if(poolEn){
    offset = (posY*fmRow+posX)*biasNUM*49 + channelIdx*49;
  } else {
    offset = (posY*fmRow+posX)*biasNUM*49*4 + channelIdx*49*4 + quarterIdx*49;
  }
  offset = offset*sizeof(unsigned short);
  if(wrEnable) {
    fseek((FILE*)fileDescriptor, offset, SEEK_SET);
    fwrite(wrData, sizeof(unsigned short), 49, (FILE*)fileDescriptor);
  }
  return;
}

void write2FileFixed29(void* fileDescriptor, U wrEnable, U poolEn, U posX, U posY, U quarterIdx, U channelIdx, U* wrData)
{
  unsigned int offset;
  if(poolEn){
    offset = (posY*fmRow+posX)*biasNUM*49 + channelIdx*49;
  ...
  if(wrEnable) {
    fseek((FILE*)fileDescriptor, offset, SEEK_SET);
    fwrite(wrData,sizeof(unsigned int), 49,  (FILE*)fileDescriptor);
  }
  return;
}

void write2FileExp5(void* fileDescriptor, U wrEnable, U poolEn, U posX, U posY, U quarterIdx, U channelIdx, U* wrData)
{
  unsigned int offset;
  if(poolEn){
    offset = (posY*fmRow+posX)*biasNUM + channelIdx;
  } else {
    offset = (posY*fmRow+posX)*biasNUM*4 + channelIdx*4 + quarterIdx;
  }
  offset = offset*sizeof(unsigned char);
  if(wrEnable) {
    fseek((FILE*)fileDescriptor, offset, SEEK_SET);
    fwrite(wrData, sizeof(unsigned char), 1, (FILE*)fileDescriptor);
  }
  return;
}
