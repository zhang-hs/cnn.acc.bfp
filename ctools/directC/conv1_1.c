...
void write2File(void* fileDescriptor, U wrEnable, U poolEn, U posX, U posY, U quarterIdx, U channelIdx, U* wrData)
{
  unsigned int offset;
  if(poolEn){
    offset = (posY*row+posX)*biasNUM*49 + channelIdx*49;
  } else {
    offset = (posY*row+posX)*biasNUM*49*4 + channelIdx*49*4 + quarterIdx*49;
  }
...
void write2FileFixed29(void* fileDescriptor, U wrEnable, U poolEn, U posX, U posY, U quarterIdx, U channelIdx, U* wrData)
{
  ...
  if(wrEnable) {
    fseek((FILE*)fileDescriptor, offset, SEEK_SET);
    fwrite(wrData, 49, sizeof(unsigned int), (FILE*)fileDescriptor);
  }
  return;
}
...
void write2FileExp5(void* fileDescriptor, U wrEnable, U poolEn, U posX, U posY, U quarterIdx, U channelIdx, U* wrData)
{
  unsigned int offset;
  if(poolEn){
    offset = (posY*row+posX)*biasNUM + channelIdx;
  } else {
    offset = (posY*row+posX)*biasNUM + channelIdx + quarterIdx;
  }
  offset = offset*sizeof(unsigned char);
  if(wrEnable) {
    fseek((FILE*)fileDescriptor, offset, SEEK_SET);
    fwrite(wrData, 1, sizeof(unsigned char), (FILE*)fileDescriptor);
  }
  return;
}
