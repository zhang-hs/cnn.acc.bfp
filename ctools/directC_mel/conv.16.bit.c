...
#define fmHeight   224  // <-x
#define fmWidth    224  // <-x
...
#define biasMax    512
#define epsilonError  4.0
#define relativeError 0.1
#define absError      0.4
      ...
      for(int i=0; i<channelNum; i++){
        for(int r=0; r<atomicHeight/2; r++) {
          for(int c=0; c<atomicWidth/2; c++) {
            ...
            if(uiPrintPos) {
              printf("*top data error: channelNum: %8d, row: %8d, col: %8d, verilog: %8.8x, directC: %8.8x, fError=%8.6f, rError0=%8.6f, rError1=%8.6f\n", i, r, c,
                          uipVerilog[channelNum*atomicHeight*atomicWidth-1 - (i*atomicHeight*atomicWidth + r*atomicWidth + c)],
                          directCResult[i*atomicHeight/2*atomicWidth/2 + r*atomicWidth/2 + c], (*f0)-(*f1), fabs((*f0)-(*f1))/(*f0), fabs((*f0)-(*f1))/(*f1));
...
// convert 32bit float point data to 16bit float point data
void convFloat2ShortFloat(U floatVerilog, U* pShortFloat)
//void convFloat2ShortFloat(unsigned int floatVerilog, unsigned short* pShortFloat)
{
  unsigned int floatData;
  memcpy(&floatData, &floatVerilog, sizeof(unsigned int));
  unsigned int contentVerilog, signVerilog, expVerilog;
  contentVerilog  = floatData & 0x7fffffff;
  signVerilog     = floatData & 0x80000000;
  expVerilog      = floatData & 0x7f800000;
  ...
  contentVerilog  = (expVerilog < 0x38800000) ? 0 : contentVerilog;
  contentVerilog  = (expVerilog > 0x47000000) ? 0x7bff : contentVerilog;
...
// convert 16bit float point data to 32bit float point data
void convShortFloat2Float(U floatVerilog, U* pFloat)
{
  unsigned short floatData;
  memcpy(&floatData, &floatVerilog, sizeof(unsigned short));
  unsigned int contentVerilog, signVerilog, expVerilog;
  contentVerilog  = floatData & 0x7fff;
  signVerilog     = floatData & 0x8000;
  expVerilog      = floatData & 0x7c00;

  contentVerilog  <<= 13;
  signVerilog     <<= 16;
  if(expVerilog == 0x7c00) {
    contentVerilog  = 0x7fffffff;
  } else {
    contentVerilog  += 0x38000000;
    contentVerilog  = (expVerilog == 0) ? 0 : contentVerilog;
  }

  contentVerilog |= signVerilog;
  *pFloat = contentVerilog;
}
