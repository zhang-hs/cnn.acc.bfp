...
//get 224x224 picture from 640x480 video, only Y is need.
unsigned int
yuv640toy224(unsigned char * input_ptr, unsigned char * output_ptr, unsigned int image_width, unsigned int image_height)
{
	...
	buff += (640*2*14+1088);
	for (i=image_height; i>0; i--)
	{
		buff += 1664;	//(192+1280+192)
		//buff += 384+448+1280;
		for(j=image_width; j>0; j--)
		{
			*output_pt++ = buff[0];
			buff += 4;
		}
...
//get 224x224 rgb picture from 640x480 yuyv video.
unsigned int
yuv640torgb224(unsigned char * input_ptr, unsigned char * output_ptr, unsigned int image_width, unsigned int image_height)
{
	unsigned int i, j, size;
	...
	pic_rgb = (unsigned char *)malloc(640*480*3*sizeof(char));
	Pyuv422torgb24(input_ptr, pic_rgb, 640, 480);
	//pic_rgb -= 640*480*3;
...
//write a picture to FPGA with dma
static int dma_to_device(char *device, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filein)
{
  int rc;
  char *buffer = NULL;
  char *allocated = NULL;
  ...
  posix_memalign((void **)&allocated, 4096/*alignment*/, size + 4096);//对齐后分配内存，返回所分配地址
  assert(allocated);
  buffer = allocated + offset;//加偏移
...
static int transImage( float* OrigData, float* TransData ) {
...
static void to_float16(float* DataFp32, uint16_t* TransFp16, uint8_t* Exp, int FloatLen) {
...
int pic_to_device(unsigned char *buf,int width,int height,char *device,uint32_t addr,uint32_t offset,uint32_t count, char* exp_data)
{
FILE *foutpict;
unsigned char *picture = NULL;
float *pic_sub_mean = NULL;
char *name = NULL;
name = calloc(80,1);
getPictureName (name, 0);
picture = (unsigned char *)malloc(width*height*3*sizeof(unsigned char));
pic_sub_mean = (float *)malloc(width*height*3*sizeof(float));
if(picture){
	//yuv640toy224(buf, picture, width, height);
	yuv640torgb224(buf, picture, width, height);
}else{
	printf(" no room to take a picture \n");
	return 0;
}
...
{
	*pic_sub_mean++ = (float)picture[2]-104.0;
	picture += 3;
}
picture -= 3*width*height;
for(unsigned int j=width*height; j>0; j--)
{
	*pic_sub_mean++ = (float)picture[0]-117.0;
	picture += 3;
}
picture -= 3*width*height;
for(unsigned int k=width*height; k>0; k--)
{
	*pic_sub_mean++ = (float)picture[1]-123.0;
	picture += 3;
}
picture -= 3*width*height;
pic_sub_mean -= 3*width*height;

// Transform image storage

float* pic_trans = (float *) malloc( TRANS_SIZE*TRANS_SIZE*3*sizeof(float) );
transImage(pic_sub_mean, pic_trans);

// Transform to float16
uint16_t* pic_fp16 = (uint16_t *) malloc( TRANS_SIZE * TRANS_SIZE * 3 * sizeof(uint16_t));
uint8_t* exp_pic = (uint8_t*)malloc(1);
char* data_wr = (char*)malloc(TRANS_SIZE * TRANS_SIZE * 3 * sizeof(uint16_t) + 8192);

to_float16(pic_trans, pic_fp16, exp_pic, TRANS_SIZE * TRANS_SIZE * 3);
printf("exp of pic: %x\n", *exp_pic);

memcpy(data_wr, pic_fp16, TRANS_SIZE * TRANS_SIZE * 3 * sizeof(uint16_t));
memcpy(data_wr+TRANS_SIZE * TRANS_SIZE * 3 * sizeof(uint16_t), exp_pic, 1);
memcpy(data_wr+TRANS_SIZE * TRANS_SIZE * 3 * sizeof(uint16_t)+64, exp_data+64, 8128);
/*
char* fn = "./data/get.txt";
int fd = open(fn, O_WRONLY | O_CREAT | O_TRUNC | O_SYNC);
write(fd, (char *)data_wr, 401408);
close(fd);
*/
dma_to_device(device, addr, 401408, offset, count, data_wr);
//for debug
/*
if(name){
	foutpict = fopen (name, "wb");
...
//get classification results from FPGA
int results_from_device(char *device, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *fpga_data)
{
  int rc;
  char *buffer = NULL;
  char *allocated = NULL;
  struct timespec ts_start, ts_end;
  char* results = fpga_data;
  ...
  while (count--) {
	...
	memcpy(results, buffer, 1000*sizeof(uint16_t));
  }
  /* subtract the start time from the end time */
  //timespec_sub(&ts_end, &ts_start);
  /* display passed time, a bit less accurate but side-effects are accounted for */
...
//do the lookup job with results and ref-file

void classify_batch(char* fpgaData, char* labelFile, char* refLabelData, char* results, char* prob_top5)
{

	char* fpga_byte = fpgaData;
	uint16_t* fpga_fp16 = (uint16_t*)fpga_byte; 
	float* fpga_fp32 = (float *)malloc(BATCH_SIZE*FILE_LEN*4);;
	char* ref_label_byte = refLabelData;
	float* ref_label_fp32 = (float*)ref_label_byte;
	...
	//int   correct_cnt = 0;
	char  out_label[1024];
	char *out_label_bg = NULL;
	char mem_offset = 0;
	out_label_bg = (char*) malloc (50*sizeof(char));
  
	FILE *label_file_handle = fopen(labelFile,"r");
	//FILE *log_file_handle = fopen("label-log-fpga.txt", "w");
	if(label_file_handle==NULL) {
	  printf("ERROR: open label file.\n");
	  return;
	}
	...
	{
	  //fprintf(log_file_handle,"%dth image classification...\n",k);
	  for(int m = 0; m < 5; m++)
	  {
		mem_offset = 0;
		max_index[m] = 0;
		...
		for(int i = 0; i < 32; i++)
		{
			for(int j = 0; j < 32; j++) 
			{
				...
				if(index < 1000)
				{
					float cur_fpga = fpga_fp32[k*1024+index];
					if( (max_prob[m] < cur_fpga) && (m==0? 1:(cur_fpga < max_prob[m-1])) ) 
					{
						max_prob[m] = cur_fpga;
						max_index[m] = index;
					}
	  ...
	  fseek(label_file_handle, 0, 0);
	  for(int l = 0; l <= max_index[m]; l++) 
	  {
		fgets(out_label, sizeof(out_label), label_file_handle);//循环执行max_index次，out_lable中存储最后一次结果
	  }
	  //turn the classification results to "char *results"
	  memcpy(out_label_bg, out_label, 50);
	  out_label_bg += 10;
	  while( (*out_label_bg != ',') &&(*out_label_bg != '\n') && ( *out_label_bg != ' ' ? 1:(*(out_label_bg+1) != ' ')) )
	  {
		out_label_bg++;
		mem_offset++;
	  }
	  results += m*30;
	  out_label_bg -= mem_offset;
	  memcpy(results, out_label_bg, mem_offset);
	  strcpy(results+mem_offset, "\0");
	  out_label_bg -= 10;
	...
	{
		deno += exp(fpga_fp32[x] - max_prob[0]);
		//printf("prob[%d]: %f\n",x,fpga_fp32[x]);
	}
	printf("deno: %f\n", deno);
	for (int y = 0; y < 5; y++)
	{
		double prob_e = exp(max_prob[y]-max_prob[0])/deno;
		sprintf(prob_top5+y*10, "%f", prob_e);
	 	printf("prob_top[%d]: %s\n", y, prob_top5+y*10 );
	}
	printf("\n");
	for (int z = 0; z < 5; z++)
	...
	// double prob_e = exp(max_prob[0]-max_prob[0])/deno;
	// //gcvt(prob_e, 6, prob_top);
	// sprintf(prob_top, "%f", prob_e);
	// printf("prob_top: %s\n", prob_top);
...
// //get classification results from FPGA
// int results_from_device(char *device, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filename)
// {
//   int rc;
//   char *buffer = NULL;
//   char *allocated = NULL;
...
// void classify_batch(char* fpgaFile, char* labelFile, char* results, char* prob_top5)
// {
...
// 	FILE *fpga_file_handle = fopen(fpgaFile, "rb");
// 	FILE *label_file_handle = fopen(labelFile,"r");
// 	FILE *ref_label_handle = fopen("./data/batch.label.bin", "rb");
// 	//FILE *log_file_handle = fopen("label-log-fpga.txt", "w");
  
// 	if(fpga_file_handle==NULL) {
// 	  printf("ERROR: open fpga file.\n");
// 	  return;
// 	}
// 	if(label_file_handle==NULL) {
// 	  printf("ERROR: open label file.\n");
...
// 	//--read byte from file
// 	fpga_byte = (char *)malloc(BATCH_SIZE*FILE_LEN*2);
// 	fpga_fp32 = (float *)malloc(BATCH_SIZE*FILE_LEN*4);
// 	ref_label_byte = (char *)malloc(BATCH_SIZE*4);
// 	assert(fpga_byte!=NULL);
// 	assert(fpga_fp32!=NULL);
// 	assert(ref_label_byte!=NULL);
  
// 	fread(fpga_byte, 1, BATCH_SIZE*FILE_LEN*2, fpga_file_handle);
// 	fread(ref_label_byte, 1, BATCH_SIZE*4, ref_label_handle);
  
// 	//--transform to normal
// 	fpga_fp16 = (uint16_t*)fpga_byte;
// 	to_float32(fpga_fp32, fpga_fp16);
// 	ref_label_fp32 = (float*)ref_label_byte;
