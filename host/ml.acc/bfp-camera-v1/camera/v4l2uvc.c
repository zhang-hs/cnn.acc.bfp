...
int v4L2UpDownPanTilt(struct vdIn *vd, short inc_p, short inc_t) {
  ...
  control_s[0].u.value = inc_p;
  control_s[1].id = t_control;
  control_s[1].u.value = inc_t;
...
int enum_frame_intervals(int dev, __u32 pixfmt, __u32 width, __u32 height)
{
	...
	while ((ret = ioctl(dev, VIDIOC_ENUM_FRAMEINTERVALS, &fival)) == 0) {
		if (fival.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
				printf("%u/%u, ",
						fival.u.discrete.numerator, fival.u.discrete.denominator);
		} else if (fival.type == V4L2_FRMIVAL_TYPE_CONTINUOUS) {
				printf("{min { %u/%u } .. max { %u/%u } }, ",
						fival.u.stepwise.min.numerator, fival.u.stepwise.min.numerator,
						fival.u.stepwise.max.denominator, fival.u.stepwise.max.denominator);
				break;
		} else if (fival.type == V4L2_FRMIVAL_TYPE_STEPWISE) {
				printf("{min { %u/%u } .. max { %u/%u } / "
						"stepsize { %u/%u } }, ",
						fival.u.stepwise.min.numerator, fival.u.stepwise.min.denominator,
						fival.u.stepwise.max.numerator, fival.u.stepwise.max.denominator,
						fival.u.stepwise.step.numerator, fival.u.stepwise.step.denominator);
				break;
		}
		fival.index++;
	}
...
int enum_frame_sizes(int dev, __u32 pixfmt)
{
	...
	while ((ret = ioctl(dev, VIDIOC_ENUM_FRAMESIZES, &fsize)) == 0) {
		if (fsize.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
			...
			// 		fsize.discrete.width, fsize.discrete.height);
			ret = enum_frame_intervals(dev, pixfmt,
					fsize.u.discrete.width, fsize.u.discrete.height);
			if (ret != 0)
				printf("  Unable to enumerate frame sizes.\n");
		} else if (fsize.type == V4L2_FRMSIZE_TYPE_CONTINUOUS) {
			printf("{ continuous: min { width = %u, height = %u } .. "
					"max { width = %u, height = %u } }\n",
					fsize.u.stepwise.min_width, fsize.u.stepwise.min_height,
					fsize.u.stepwise.max_width, fsize.u.stepwise.max_height);
			printf("  Refusing to enumerate frame intervals.\n");
			break;
		} else if (fsize.type == V4L2_FRMSIZE_TYPE_STEPWISE) {
			printf("{ stepwise: min { width = %u, height = %u } .. "
					...
					fsize.u.stepwise.min_width, fsize.u.stepwise.min_height,
					fsize.u.stepwise.max_width, fsize.u.stepwise.max_height,
					fsize.u.stepwise.step_width, fsize.u.stepwise.step_height);
			printf("  Refusing to enumerate frame intervals.\n");
			break;
		}
		fsize.index++;
