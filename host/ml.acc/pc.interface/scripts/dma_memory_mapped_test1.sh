...
if [ $h2cChannels -gt 0 ]; then
  ...
  do
   ...
   #./dma_to_device -d /dev/xdma0_h2c_${curChannel} -f data/datafile${i}_4K.bin -s $transferSize -a $addrOffset -c $transferCount &
    ./dma_to_device -d /dev/xdma0_h2c_${curChannel} -f data/16.bit.conv1_1.bottom.txt -s $transferSize -a $addrOffset -c $transferCount &
...
if [ $c2hChannels -gt 0 ]; then
  ...
  do
    ...
    ./dma_from_device -d /dev/xdma0_c2h_${curChannel} -f data/output_conv.bin -s $transferSize -a $addrOffset -c $transferCount &
    # If all channels have active transactions we must wait for them to complete
...
if [ $h2cChannels -eq 0 ]; then
  ...
  for ((i=0; i<=0; i++))
  do
    cmp data/output_conv.bin data/16.bit.conv1_1.bottom.txt -n $transferSize
    returnVal=$?
    if [ ! $returnVal == 0 ]; then
